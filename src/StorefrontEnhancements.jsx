...
export function ProductCatalog({ products = [], go, standalone = false }) {
  ...
  const available = liveProducts.filter(product => product?.active !== false && product?.active !== 0).map(product=>({...product,category:product.category?.trim()||'Custom'}))
  const categories = useMemo(() => [...new Set([...categoryOrder,...available.map(p=>p.category)])].filter(category => available.some(product => product.category === category)), [available])
  ...
  return <section className="catalog-section" id="catalog">
    ...
    <div className="catalog-filters" aria-label={am?'የምርት ምድቦች':'Product categories'}><button aria-pressed={category==='All'} className={category === 'All' ? 'active' : ''} onClick={() => setCategory('All')}>{am?'ሁሉም ምርቶች':'All products'}</button>{categories.map(item => <button aria-pressed={category===item} className={category === item ? 'active' : ''} key={item} onClick={() => setCategory(item)}>{categoryLabels[item] || item}</button>)}</div>
    {categories.filter(group => category==='All' || group===category).map(group => <section className="catalog-group" key={group}><h3 className="catalog-group-title">{categoryLabels[group]||group}<small>{available.filter(p=>p.category===group).length}</small></h3><div className="catalog-grid">{available.filter(product=>product.category===group).map(product => <article className="catalog-card" key={product.id || product.slug}><div className="catalog-icon"><Package size={20}/></div><div className="eyebrow">{categoryLabels[product.category] || product.category}</div><h4>{product.name.replace(/ package$/i, '')}</h4><p>{product.description || categoryCopy[product.category] || (am?'በMaxrez ቡድን እንደ ፍላጎትዎ የተዘጋጀ።':'Made to your specification by the Maxrez team.')}</p><div className="catalog-meta"><strong>{money(product.base_price)}<small>{Number(product.base_price) > 0 ? (am?'መነሻ ዋጋ':'starting point') : ''}</small></strong><span>{am?'ከ':'From'} {product.minimum_quantity || 1} {am?'ክፍሎች':'units'}</span></div><button className="text-link" onClick={() => go(`/order?product=${encodeURIComponent(product.slug)}`)}>{am?'ይህን ያዘጋጁ':'Configure this'} <ChevronRight size={16}/></button></article>)}</div></section>)}
