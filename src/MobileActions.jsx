...
export default function MobileActions({go,path,menu}) {
 ...
 return <nav className="mobile-actions" aria-label="Quick actions"><button onClick={()=>go('/order')}><ShoppingBag size={19}/>Order</button><button onClick={()=>{go('/');setTimeout(()=>document.querySelector('.file-drop-section')?.scrollIntoView({behavior:'smooth'}),50)}}><FileUp size={19}/>Send files</button><a href="tel:+251911207630"><Phone size={19}/>Call</a></nav>
}
