...
app.post('/api/setup', async (req,res) => { const { setupKey,adminName,adminEmail,adminPassword,paymentMethods=[] } = req.body; if (!process.env.SETUP_KEY || setupKey !== process.env.SETUP_KEY) return res.status(403).json({ error:'Invalid setup key' }); if (!adminName || !adminEmail || !adminPassword || adminPassword.length < 10) return res.status(400).json({ error:'Complete all admin fields and use at least 10 password characters' }); await ensureSchema(); const [existing] = await pool.query('SELECT COUNT(*) count FROM users'); if (Number(existing[0].count)) return res.status(409).json({ error:'Setup has already been completed' }); const hash = await bcrypt.hash(adminPassword,12); await pool.query('INSERT INTO users (name,email,password_hash,role) VALUES (?,?,?,"admin")',[adminName,adminEmail,hash]); for (const method of paymentMethods.filter(x=>x.name&&x.accountLabel)) await pool.query('INSERT INTO payment_methods (name,instructions,account_label) VALUES (?,?,?)',[method.name,method.instructions||'Upload your receipt after payment.',method.accountLabel]); res.json({ ok:true }) })
