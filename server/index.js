...
app.get('/api/payment-methods', async (_req,res) => { const [rows]=await pool.query('SELECT id,name,instructions,account_label,qr_url FROM payment_methods WHERE active=1 ORDER BY sort_order,id'); const existing=new Set(rows.map(x=>x.name.toLowerCase())); for(const name of ['Telebirr','CBE Birr','Awash Bank']) if(!existing.has(name.toLowerCase())) await pool.query('INSERT INTO payment_methods (name,instructions,account_label,sort_order) VALUES (?,"Send the payment receipt with your order number.","",?)',[name,rows.length]); const [updated]=await pool.query('SELECT id,name,instructions,account_label,qr_url FROM payment_methods WHERE active=1 ORDER BY sort_order,id'); res.json(updated) })
...
app.post('/api/admin/payment-methods/:id/qr', auth, roles('admin'), upload.single('qr'), async (req,res) => { if(!req.file)return res.status(400).json({error:'Choose a QR image'}); const url=`/api/payment-qr/${req.file.filename}`; await pool.query('UPDATE payment_methods SET qr_url=? WHERE id=?',[url,req.params.id]); res.json({ok:true,qrUrl:url}) })
app.get('/api/payment-qr/:file', (req,res) => { const safe=path.basename(req.params.file); const file=path.join(uploadDir,safe); if(!fs.existsSync(file))return res.sendStatus(404); res.sendFile(file) })
