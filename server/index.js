...
import { ensureWorkflow, registerWorkflow } from './workflow.js'
...
const allowedOrigins = (process.env.FRONTEND_ORIGIN || 'https://maxrez.cc,http://localhost:5173,http://127.0.0.1:5173,http://localhost:3001,http://127.0.0.1:3001').split(',').map(x => x.trim()).filter(Boolean)
...
const productSeeds = [
  ['business-cards','Business Cards','Cards & stationery',4.5,50],['brochures','Brochures & Flyers','Marketing print',8,25],['labels','Labels & Stickers','Labels',3.5,50],['books','Books & Catalogues','Publishing',65,10],['packaging','Boxes & Packaging','Packaging',35,25],['restaurant-print','Restaurant Print Pack','Industry bundles',28,25],['large-format','Banners & Large Format','Large format',180,1],['custom','Custom Print Project','Custom',1,1],['ctp-plates','CTP Plates','Prepress',450,1],['film-output','Film Output','Prepress',300,1],['packaging-labels','Packaging Labels','Labels',3.5,25],['cosmetic-cartons','Cosmetic Cartons','Packaging',35,25],['pouches','Custom Pouches','Packaging',22,25],['paper-bags','Branded Paper Bags','Packaging',25,50],['menus','Menu Printing','Marketing print',20,25],['hang-tags','Hang Tags','Labels',5,50],['notebooks','Notebooks','Publishing',35,25]
]
...
async function ensureSchema() {
  ...
  for (const [i,[slug,name,category,base,min]] of productSeeds.entries()) { const [existing]=await pool.query('SELECT id FROM product_catalog WHERE slug=?',[slug]); if(!existing.length) await pool.query('INSERT INTO product_catalog (slug,name,description,category,base_price,minimum_quantity,pricing_rules,sort_order) VALUES (?,?,?,?,?,?,?,?)',[slug,name,`Configure ${name.toLowerCase()} and receive an instant estimate.`,category,base,min,JSON.stringify(productRules(slug,defaultRules)),70+i]); }
  const [tierCount] = await pool.query('SELECT COUNT(*) count FROM delivery_zones WHERE min_km IS NOT NULL')
  const [specMigration]=await pool.query('SELECT name FROM workflow_migrations WHERE name="product-specifications-v1"')
  if(!specMigration.length){
...
app.get('/api/customers/notifications', customerAuth, async (req,res) => { const [rows]=await pool.query('SELECT e.id,e.status,e.note,e.created_at,o.public_id,o.tracking_token,o.service FROM order_events e JOIN orders o ON o.id=e.order_id WHERE o.customer_id=? OR (o.customer_email=? AND o.customer_email IS NOT NULL) OR o.customer_phone=? ORDER BY e.created_at DESC LIMIT 40',[req.user.id,req.user.email,req.user.phone]); res.json(rows.map(row=>({...row,title:`${row.public_id} · ${String(row.status).replaceAll('_',' ')}`}))) })
...
registerWorkflow(app, { pool, auth, roles, customerAuth, uploadDir, uploadLimit })
...
async function saveLeadSubmission(req, res, label) {
  const name = String(req.body.name || '').trim()
  const phone = String(req.body.phone || '').trim()
  const email = normalizeEmail(req.body.email)
  const note = String(req.body.note || '').trim().slice(0, 4000)
  if (!name || (!phone && !email) || !note) return res.status(400).json({ error: 'Add your name, phone or email, and a short description.' })
  try { const [result] = await pool.query('INSERT INTO file_drop_submissions (customer_name,customer_phone,customer_email,note) VALUES (?,?,?,?)', [name, phone || null, email || null, `${label}\n${note}`]); res.status(201).json({ ok:true, id:`${label.replaceAll(' ','-')}-${result.insertId}` }) } catch (error) { console.error(`${label} failed:`, error.message); res.status(500).json({ error: 'Unable to send your request right now.' }) }
}
app.post('/api/design-briefs', dropLimit, (req,res) => saveLeadSubmission(req,res,'DESIGN BRIEF'))
app.post('/api/sample-requests', dropLimit, (req,res) => saveLeadSubmission(req,res,'SAMPLE PACK REQUEST'))
...
async function calculateProjectQuote(items, fallbackProduct) {
  const validItems = Array.isArray(items) ? items.slice(0, 20) : []
  const slugs = [...new Set(validItems.map(item => String(item.slug || '').trim()).filter(Boolean))]
  if (!slugs.length) return null
  const [rows] = await pool.query('SELECT * FROM product_catalog WHERE slug IN (?) AND active=1', [slugs])
  const bySlug = new Map(rows.map(row => [row.slug, row]))
  const lineItems = validItems.map(item => { const product = bySlug.get(item.slug); if (!product) throw Object.assign(new Error('One project item is no longer available.'), { statusCode: 400 }); const line = calculateQuote(product, parseJson(item.form, {})); return { slug: product.slug, name: product.name, quantity: line.quantity, production: line.production, designFee: line.designFee, urgentFee: line.urgentFee, subtotal: line.subtotal, selections: line.selections } })
  const subtotal = lineItems.reduce((sum, item) => sum + Number(item.subtotal || 0), 0)
  return { quantity: 1, unitPrice: Number(subtotal.toFixed(2)), production: Number(lineItems.reduce((sum, item) => sum + Number(item.production || 0), 0).toFixed(2)), designFee: Number(lineItems.reduce((sum, item) => sum + Number(item.designFee || 0), 0).toFixed(2)), urgentFee: Number(lineItems.reduce((sum, item) => sum + Number(item.urgentFee || 0), 0).toFixed(2)), subtotal: Number(subtotal.toFixed(2)), items: lineItems, selections: { basket: true } }
}
...
app.post('/api/orders', uploadLimit, upload.array('artwork',5), async (req,res) => {
  ...
  const [products]=await pool.query('SELECT * FROM product_catalog WHERE id=? AND active=1',[req.body.productId]); let product=products[0]; if(!product){ files.forEach(file=>fs.unlink(file.path,()=>{})); return res.status(400).json({error:'Choose a valid product'}) }
  let quote; try { quote = details.basketItems?.length ? await calculateProjectQuote(details.basketItems, product) : calculateQuote(product, details) } catch (error) { files.forEach(file=>fs.unlink(file.path,()=>{})); return res.status(error.statusCode || 400).json({ error: error.message }) }
  if (details.basketItems?.length) product = { ...product, name: `Project quote · ${details.basketItems.length} items` }
  const delivery=fulfillment==='delivery'?await deliveryEstimate({address:req.body.deliveryAddress,zone:req.body.deliveryZone,urgent,lat:req.body.lat,lng:req.body.lng}):{source:'pickup',durationSeconds:0,distanceMeters:0,fee:0}, reports=[]
