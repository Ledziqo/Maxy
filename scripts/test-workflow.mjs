...
try {
 ...
 const [[verifiedBefore]]=await db.query('SELECT payment_verified_at FROM orders WHERE id=?',[id])
 await json(`/orders/${id}/payment`,{paymentStatus:'verified',paymentMethod:'Telebirr'},admin,'PATCH')
 const [[verifiedAgain]]=await db.query('SELECT status,payment_verified_at,payment_method FROM orders WHERE id=?',[id])
 assert.equal(verifiedAgain.status,'completed','Re-verifying must not move a completed order backwards')
 assert.equal(verifiedAgain.payment_verified_at.getTime(),verifiedBefore.payment_verified_at.getTime(),'Keep the original verification date')
 assert.equal(verifiedAgain.payment_method,'Telebirr')
